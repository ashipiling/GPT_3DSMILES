...
def gen_pocket_str(
    ...
    try:
        ...
        # 打印出selected的氨基酸中的每个原子
        for nn in selected:
            tmp_dict_list.append( list(nn.getCoords()) )

        tmp_dict_list = np.array(tmp_dict_list)

        dist_mat = distance_matrix(ligand_coords, tmp_dict_list)
        # 找到最近的26个原子

        min_dist_list_set = set()
        for dist_list in dist_mat:
            tmp = np.argsort(dist_list)
            min_dist_list_set.add(tmp[0])
            min_dist_list_set.add(tmp[1])
        final_list = list(min_dist_list_set)[:26]
        for index in final_list:
            pocket_res_str += 'X'
            pocket_coord_str += transform_coord(tmp_dict_list[index], coord_offset=coord_offset, rot_matrix=rot_matrix)
...
if __name__ == '__main__':
    ...
    out_3dsmiles_path = 'data/pocketsmiles_crossdocked_with_property_9.txt'
