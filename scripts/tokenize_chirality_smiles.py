...
class Vocabulary(object):
    ...
    def tokenize(self, smiles):
        ...
        tokenized.append('&')
        smiles = self.decode(tokenized)
        return smiles

    def charity_tokenizer(self,smi):
        """
...
def construct_vocabulary(smiles_list):
    ...
    with open('Voc_SMILES', 'w') as f:
        for char in add_chars:
            f.write(char + "\n")
    return add_chars


def construct_charity_vocabulary(smiles_list):
    """Returns all the characters present in a SMILES file.
       Uses regex to find characters/tokens of the format '[x]'."""
    add_chars = set()
    for i, smiles in enumerate(smiles_list):
        pattern = "(\[[^\]]+]|Br?|Cl?|N|O|S|P|F|I|b|c|n|o|s|p|\(|\)|\.|=|#|-|\+|\\\\|\/|:|~|@|\?|>|\*|\$|\%[0-9]{2}|[0-9])"
        regex = re.compile(pattern)
        char_list = [token for token in regex.findall(smiles)]

        for item in char_list:
            add_chars.add(item)

    print("Number of characters: {}".format(len(add_chars)))
    with open('Voc_charity_SMILES', 'w') as f:
        for char in add_chars:
            f.write(char + "\n")
    return add_chars
...
def find_wrong_token(smiles_list):
    wrong_chars = ['[1C@H]','[15n]','[1C@@H]']
    count = 0
    for i, smiles in enumerate(smiles_list):
        pattern = "(\[[^\]]+]|Br?|Cl?|N|O|S|P|F|I|b|c|n|o|s|p|\(|\)|\.|=|#|-|\+|\\\\|\/|:|~|@|\?|>|\*|\$|\%[0-9]{2}|[0-9])"
        regex = re.compile(pattern)
...
if __name__ == "__main__":
    ...
    # smiles_list = canonicalize_smiles_from_file('./DRUGS/geo_drug_smiles.csv')

    smiles_list = pd.read_csv('./smiles_all.txt').values.flatten().tolist()

    find_wrong_token(smiles_list)

    print("Constructing vocabulary...")
    voc_chars = construct_charity_vocabulary(smiles_list)
    # write_smiles_to_file(smiles_list, "zinc_mols_filtered.smi")



    # voc = Vocabulary(init_from_file="Voc_SMILES")
    # zinc_mols = pd.read_csv('zinc_mols_filtered.smi',header=None).values.flatten().tolist()
    # for i in zinc_mols:
    #     tokenized = voc.tokenize(i)
    #
    #     # encoded
