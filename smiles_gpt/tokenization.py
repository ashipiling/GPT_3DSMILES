...
__all__ = ("PAD_TOKEN", "BOS_TOKEN", "EOS_TOKEN", "UNK_TOKEN", "SUFFIX",
           ...
           "UNK_TOKEN_ID", "SMILESBPETokenizer", "SMILESAlphabet", "TDSMILESAlphabet")
...
@dataclass(init=True, eq=False, repr=True, frozen=True)
class TDSMILESAlphabet(Collection):
    atoms: FrozenSet[str] = frozenset([
        'Ac', 'Ag', 'Al', 'Am', 'Ar', 'As', 'At', 'Au', 'B', 'Ba', 'Be', 'Bh',
        'Bi', 'Bk', 'Br', 'C', 'Ca', 'Cd', 'Ce', 'Cf', 'Cl', 'Cm', 'Co', 'Cr',
        'Cs', 'Cu', 'Db', 'Dy', 'Er', 'Es', 'Eu', 'F', 'Fe', 'Fm', 'Fr', 'Ga',
    ...
    non_atoms: FrozenSet[str] = frozenset([
        ...
        '1', '2', '3', '4', '5', '6', '7', '8', '9', '0',
        '@', '{', '|', '&', '%', '^',
        'qed0', 'qed1', 'logp0', 'logp1', 'vina0', 'vina1'
    ])

    # -1999 ~ 1999
    coord: FrozenSet[str] = frozenset(coord_list)

    additional: FrozenSet[str] = frozenset()

    def __contains__(self, item: Any) -> bool:
        return item in self.atoms or item in self.non_atoms or item in self.coord

    def __iter__(self):
        return (token for token in chain(self.atoms, self.non_atoms, self.coord))

    def __len__(self) -> int:
        return len(self.atoms) + len(self.non_atoms) + len(self.coord) + len(self.additional)
