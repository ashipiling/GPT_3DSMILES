...
def gen_3dsmiles(mol, if_train=True):
    # smiles 过滤
...
#         resname_list = []
#         for nn in selected:
#             vertice_list.append(list(nn.getCoords()))
#             resname_list.append(nn.getResname())
        
#         dist_mat = distance_matrix(ligand_coords, np.array(vertice_list))
#         min_dist_list_set = []
...
#         final_list = min_dist_list_set
#         ver_list = []
#         res_list = []
#         for id in final_list:
#             ver_list.append(vertice_list[id])
#             res_list.append(resname_list[id])

#         if if_expand and ori_xscore > -7:
#             ver_list = expand_vertices(ver_list, ligand_coords)

#         # print('final_vertice_list', len(ver_list))
#         # print('ver_list', ver_list, res_list)

#         for index in range(len(ver_list)):
#             coord = copy.deepcopy(ver_list[index])
#             resname = res_list[index]
...
def gen_pocket_str(
    ...
    for dist_list in dist_mat:
        ...
        if len(tmp) > 1 and not tmp[1] in min_dist_list_set:
            min_dist_list_set.append(tmp[1])
        if len(tmp) > 2 and not tmp[2] in min_dist_list_set and ligand_mol.GetNumAtoms() < 26:
    ...
    for f_vert in ver_list:
        # pocket_res_str += 'X'
        pocket_coord_str += transform_coord(f_vert, coord_offset=coord_offset, rot_matrix=rot_matrix)
...
def expand_vertices(vertice_list, ligand_coords):
    ret_list = []
    for vertice in vertice_list:
        nearest_coord = find_nearest_coord(vertice, ligand_coords)
        new_vertice = expand_vertice(vertice, nearest_coord, offset=1.5)
...
def expand_vertice(A, B, offset=2):
    # 计算BA向量的坐标差值
    dx = B[0] - A[0]
    dy = B[1] - A[1]
    dz = B[2] - A[2]
...
def gen_3dsmiles_list(pocket_ligand_dict, add_xscore=True):
    ...
    for index, pocket_ligand_dict in enumerate(pocket_ligand_dict_list):
        ...
        _mol = pocket_ligand_dict['ligand_mol']
        ...
        # 获取蛋白表面点
        vertice, errno = get_pocket_vertice(pocket_path, _mol.GetConformer().GetPositions(), config=config)
        
        if errno:
            print('get pocket vertice failed: {}'.format(pocket_path))
            continue
    
        for dist_t in [5.5, 6, 6.5, 7, 7.5, 8]:
            ...
            xscore_string = '|' + str(xscore)
            if add_xscore:
                smiles_3d_list.append(pocket_str + '|' + str(xscore_flag) + '|' + property_str + '|' + ligand_str + xscore_string)
            else:
                smiles_3d_list.append(pocket_str + '|' + str(xscore_flag) + '|' + property_str + '|' + ligand_str)
...
def main(pocket_path, sdf_path, out_3dsmiles_path, n_worker=32):
    path_dict_list = []
    mols = Chem.SDMolSupplier(sdf_path)
    print('all mols', len(mols))
    
    for index, mol in enumerate(mols):
        try:
            vina = float(mol.GetProp('Vina Score'))
        except:
            print('block', Chem.MolToMolBlock(mol))
            print('index', index)
...
if __name__ == '__main__':
    # pocket_path = 'data/redisc_mol/egfr_10/egfr_5d41.pdb'
    pocket_path = 'data/8gur.pdb'
    # sdf_path = 'data/redisc_mol/egfr_10/egfr_train_2.sdf'
    sdf_path = 'data/cb2-3000-luohao.sdf'
    # out_3dsmiles_path = 'data/pocketsmiles_redisc_egfr_10_2_vertices.txt'
    out_3dsmiles_path = 'data/pocketsmiles_8gur_vertices.txt'

    main(pocket_path, sdf_path, out_3dsmiles_path)
