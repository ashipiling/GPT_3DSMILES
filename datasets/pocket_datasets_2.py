...
class FragSmilesPocketPretrainDataset():
    def __init__(self, cfg, mode='train'):
        ...
        self.data = pickle.load(open(cfg.DATA.offline_data, 'rb'))[mode]
    ...
    def __getitem__(self, item):
        data = self.data[item]
        res_coords = torch.tensor(data['res_coords'], dtype=torch.float32)
        ...
        res_dis = self.get_distance_matrix(res_coords)

        return {
                'smiles': data['smiles_3d'], 
                'res_seq': res_seq, 
                'res_dis': res_dis,
