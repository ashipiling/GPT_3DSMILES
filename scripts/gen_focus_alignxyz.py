...
from tokenize_chirality_smiles import Vocabulary

VOC = Vocabulary(init_from_file="scripts/Voc_charity_SMILES")
...
def save_vertices(small_new_v, mol_final_coords):
    mat = distance_matrix(mol_final_coords, small_new_v)
    # 找到距离最近的6个点的索引
    nearest_indices = np.argsort(mat, axis=1)[:, :6]
    
    # 使用索引来获取最近的6个点
    nearest_vertices = [small_new_v[indices] for indices in nearest_indices]
...
def process_vertices(vertices, mol_final_coords, rotation_matrix, translation_vector):
    big_size = 4
    big_vertices = cubefy(vertices, cube_size=big_size)
    small_size = 1.9
    small_vertices = cubefy(vertices, cube_size=small_size)
    big_new_v = np.dot((big_vertices - translation_vector), rotation_matrix.T)
    small_new_v = np.dot((small_vertices - translation_vector), rotation_matrix.T)

    big_new_v = sorted(big_new_v.tolist())

    small_v_list = save_vertices(small_new_v, mol_final_coords)

    # if err:
    #     return None
    # new_v = remove_some_vertices(new_v)
    return big_new_v, small_v_list
...
def split_str(string, pocket=False):
    string = string.replace('{', ' { ')
    string = string.replace(',', ' , ')
    if pocket:
        return string[1: ]
    else:
        qed, logp, smiles_and_xyz = string.split('|')
        smiles, mol_xyz = smiles_and_xyz.split('&')
        # mol_xyz = " ^".join(mol_xyz.split('^'))
        smiles = VOC.tokenize(smiles)
        # smiles = " ".join(smiles)

        ret = " ".join(['qed' + qed, '|', 'logp' + logp, '|', smiles, mol_xyz[1:]])
        if len(ret.split(" ")) > 1020:
            return None
    return ret


def repeat_small_list(
        splited_string, 
        splited_vertices_coords_str, 
        small_v_list
    ):
    ret = splited_vertices_coords_str + '$'
    for index, i in enumerate(small_v_list[:-1]):
        ret += split_str(coords_2_str(small_v_list[index]), pocket=True) + '$'
    
    ret_index = ''
    meet_coords = 0

    l = splited_string.split(' ')
    for i in l:
        if i.startswith('{'):
            meet_coords += 1

        ret_index += str(max(0, meet_coords - 1)) + ','
...
def gen_text(input_dict):
    ...
    for index, item in enumerate(input_dict['lst']):
        try:
            ...
            # if vertices:
            big_vertices, small_v_list = process_vertices(vertices, mol_final_coords, rotation_matrix, translation_vector)

            vertices_coords_str = coords_2_str(big_vertices)
            # else:
            # vertices_coords_str = ''
            
            prop_str = get_prop(mol)
            final_str = prop_str + '|' + canonical_smiles + '&' + ligand_coords_str
            splited_string = split_str(final_str)
            splited_vertices_coords_str = split_str(vertices_coords_str, pocket=True)
            repeat_small_v, index_string = repeat_small_list(
                splited_string, 
                splited_vertices_coords_str, 
                small_v_list
            )

            ret_list.append(repeat_small_v[:-1] + '^^^' + index_string[:-1] + '^^^' + splited_string)
        except:
            traceback.print_exc()
        if index % 1000 == 0:
            print('index', index)
...
if __name__ == '__main__':
    out_3dsmiles_path = 'data/3dsmiles_focus_alignxyz.txt'
    dir_name = 'data/pickle/'
    path_list = glob.glob(os.path.join(dir_name, 'data_batch_80*.pkl'))
